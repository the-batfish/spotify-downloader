...
class App(Tk):
    def __init__(self):
        ...
        # Setting download location defaults to Music/Spotidownload for macos and the same directory for windows and linux
        if self.config_data:
            self.location = self.config_data["location"]
        elif system() == "Darwin":
            self.location = os.path.join(
                os.path.expanduser("~/Music"), "Spotidownload"
            )
        else:
